import type { CultureEntry } from "./types";

export const russia: CultureEntry[] = [
  {
    id: "samovar",
    order: 1,
    placeholderEmoji: "🫖",
    accentColor: "#B87333",
    imageUrl: null,
    translations: {
      es: {
        title: "El ritual del té que no termina nunca",
        subtitle: "Samovar",
        imageAlt:
          "Caricatura de un samovar de metal brillante sobre una mesa, con vapor saliendo de la parte superior.",
        description:
          "El samovar es un recipiente metálico tradicional para calentar agua y preparar té, presente en los hogares rusos desde el siglo XVIII. Funciona con una especie de chimenea interna que mantiene el agua caliente durante horas, permitiendo que la tetera se quede sobre la mesa toda la tarde mientras familiares y visitas se turnan para servirse. Antes de la llegada de la electricidad, juntarse alrededor del samovar era el centro social de cualquier casa, comparable a lo que hoy sería sentarse en la cocina. Aunque los samovares eléctricos modernos reemplazaron a los de carbón, la costumbre de tomar té fuerte, servido en vasos con soporte de metal (podstakannik), sigue siendo parte central de la hospitalidad rusa.",
      },
      en: {
        title: "The Tea Ritual That Never Ends",
        subtitle: "Samovar",
        imageAlt:
          "Cartoon of a shiny metal samovar on a table, with steam rising from the top.",
        description:
          "The samovar is a traditional metal vessel for heating water and brewing tea, present in Russian homes since the 18th century. It works with an internal chimney-like tube that keeps the water hot for hours, letting the pot stay on the table all afternoon while family and guests take turns serving themselves. Before electricity arrived, gathering around the samovar was the social center of any home, comparable to what sitting in the kitchen is today. Though modern electric samovars replaced the old coal-fired ones, the custom of drinking strong tea, served in glasses held by a metal holder (podstakannik), remains central to Russian hospitality.",
      },
    },
  },
  {
    id: "matrioska",
    order: 2,
    placeholderEmoji: "🪆",
    accentColor: "#E63946",
    imageUrl: null,
    translations: {
      es: {
        title: "Muñecas que esconden muñecas",
        subtitle: "Matrioska",
        imageAlt:
          "Caricatura de un set de muñecas matrioska de distintos tamaños alineadas de mayor a menor.",
        description:
          "La matrioska, el set de muñecas de madera que se abren para revelar una más chica adentro, y así sucesivamente, es quizás el souvenir más reconocible de Rusia en todo el mundo, aunque su origen es más reciente de lo que parece: la primera se talló recién en 1890, inspirada en un set de figuras japonesas. Tradicionalmente pintadas como campesinas con pañuelo, hoy existen versiones con líderes políticos, jugadores de fútbol o personajes de dibujos animados. Más allá del souvenir turístico, la matrioska se volvió una metáfora cultural para hablar de capas ocultas de identidad, algo que muchos rusos usan para describir su propia relación con la tradición y la modernidad.",
      },
      en: {
        title: "Dolls That Hide Dolls",
        subtitle: "Matryoshka",
        imageAlt:
          "Cartoon of a set of matryoshka dolls of different sizes lined up from largest to smallest.",
        description:
          "The matryoshka, the set of wooden dolls that open to reveal a smaller one inside, and so on, is perhaps Russia's most recognizable souvenir worldwide, though its origin is more recent than it seems: the first one was carved only in 1890, inspired by a set of Japanese figures. Traditionally painted as peasant women in headscarves, versions today feature political leaders, footballers, or cartoon characters. Beyond the tourist souvenir, the matryoshka became a cultural metaphor for hidden layers of identity, something many Russians use to describe their own relationship with tradition and modernity.",
      },
    },
  },
  {
    id: "transiberiano",
    order: 3,
    placeholderEmoji: "🚆",
    accentColor: "#1D3557",
    imageUrl: null,
    translations: {
      es: {
        title: "Una semana entera cruzando un continente",
        subtitle: "El Transiberiano",
        imageAlt:
          "Caricatura de un tren cruzando un paisaje siberiano nevado con bosques de pinos a los costados.",
        description:
          "El ferrocarril Transiberiano, terminado en 1916, conecta Moscú con Vladivostok, en el extremo oriental de Rusia, a lo largo de más de 9.200 kilómetros: el trayecto en tren más largo del mundo, que cruza ocho husos horarios y puede tardar más de una semana en completarse sin escalas. Construido en tramos durante más de dos décadas, atraviesa estepas, bosques siberianos y bordea el lago Baikal, el más profundo del planeta. Para muchos viajeros, más que un medio de transporte, es una experiencia en sí misma: días enteros de paisaje cambiante, comidas compartidas con desconocidos en el vagón comedor y la sensación física de lo inmenso que es el territorio ruso.",
      },
      en: {
        title: "A Full Week Crossing a Continent",
        subtitle: "The Trans-Siberian Railway",
        imageAlt:
          "Cartoon of a train crossing a snowy Siberian landscape with pine forests on either side.",
        description:
          "The Trans-Siberian Railway, completed in 1916, connects Moscow with Vladivostok on Russia's far eastern edge, spanning more than 9,200 kilometers — the longest train journey in the world, crossing eight time zones and taking more than a week to complete without stopovers. Built in stages over more than two decades, it crosses steppes, Siberian forests, and skirts Lake Baikal, the deepest lake on the planet. For many travelers, more than just transportation, it's an experience in itself: days of changing scenery, meals shared with strangers in the dining car, and the physical sense of just how vast Russian territory really is.",
      },
    },
  },
  {
    id: "bolshoi",
    order: 4,
    placeholderEmoji: "🩰",
    accentColor: "#7B1E3A",
    imageUrl: null,
    translations: {
      es: {
        title: "El escenario que formó a las estrellas del ballet",
        subtitle: "El Bolshói",
        imageAlt:
          "Caricatura de una bailarina de ballet en puntas frente al telón de un gran teatro.",
        description:
          "El Teatro Bolshói de Moscú, fundado en 1776, es una de las compañías de ballet y ópera más prestigiosas del mundo, y una institución central en la tradición cultural rusa desde hace más de dos siglos. Su escuela formó a bailarines legendarios y estrenó obras que hoy son clásicos universales, como El lago de los cisnes y Romeo y Julieta en sus versiones más influyentes. Conseguir entradas para una función en su sala histórica, con capacidad para más de 2.000 espectadores bajo una araña de cristal gigante, sigue siendo uno de los planes culturales más codiciados de Moscú, tanto para turistas como para los propios rusos.",
      },
      en: {
        title: "The Stage That Shaped Ballet's Biggest Stars",
        subtitle: "The Bolshoi",
        imageAlt:
          "Cartoon of a ballerina en pointe in front of the curtain of a grand theater.",
        description:
          "The Bolshoi Theatre in Moscow, founded in 1776, is one of the most prestigious ballet and opera companies in the world, and a central institution in Russian cultural tradition for more than two centuries. Its school trained legendary dancers and premiered works that are today universal classics, including some of the most influential versions of Swan Lake and Romeo and Juliet. Getting tickets for a performance in its historic hall, seating more than 2,000 people under a giant crystal chandelier, remains one of Moscow's most coveted cultural outings, for tourists and Russians alike.",
      },
    },
  },
  {
    id: "san-basilio",
    order: 5,
    placeholderEmoji: "🏰",
    accentColor: "#C1272D",
    imageUrl: null,
    translations: {
      es: {
        title: "Las cúpulas que parecen de otro mundo",
        subtitle: "San Basilio",
        imageAlt:
          "Caricatura de las cúpulas coloridas y bulbosas de la Catedral de San Basilio.",
        description:
          "La Catedral de San Basilio, en la Plaza Roja de Moscú, se construyó entre 1555 y 1561 por orden de Iván el Terrible para celebrar la conquista de Kazán, y su diseño —nueve capillas distintas coronadas por cúpulas bulbosas de colores y patrones únicos— no se parece a ningún otro edificio religioso del mundo. Según una leyenda popular, sin base histórica confirmada, el zar mandó cegar al arquitecto para que nunca pudiera construir algo igual de hermoso en otro lugar. Hoy funciona como museo y es, junto con el Kremlin, la imagen más reconocible de Rusia en todo el mundo, presente en postales, películas y hasta en el logo de varias organizaciones vinculadas al país.",
      },
      en: {
        title: "The Domes That Look Like Another World",
        subtitle: "Saint Basil's Cathedral",
        imageAlt:
          "Cartoon of the colorful, bulbous domes of Saint Basil's Cathedral.",
        description:
          "Saint Basil's Cathedral, on Red Square in Moscow, was built between 1555 and 1561 by order of Ivan the Terrible to celebrate the conquest of Kazan, and its design — nine distinct chapels crowned by bulbous, colorfully patterned domes, each unique — doesn't resemble any other religious building in the world. According to a popular legend with no confirmed historical basis, the tsar had the architect blinded so he could never build anything as beautiful elsewhere. Today it functions as a museum and is, along with the Kremlin, Russia's most recognizable image worldwide, appearing on postcards, in films, and even in the logos of several organizations connected to the country.",
      },
    },
  },
  {
    id: "banya",
    order: 6,
    placeholderEmoji: "🧖",
    accentColor: "#2E5339",
    imageUrl: null,
    translations: {
      es: {
        title: "Sudar en grupo como ritual social",
        subtitle: "La banya",
        imageAlt:
          "Caricatura de una sauna de madera con piedras calientes y un manojo de ramas de abedul colgando.",
        description:
          "La banya, la sauna tradicional rusa, es mucho más que un lugar para sudar: es un ritual social que puede durar horas, alternando entre el calor intenso de la sala de vapor y el agua fría de un lago, una pileta o directamente nieve en invierno. Una parte central de la experiencia es el 'venik', un manojo de ramas de abedul o roble con hojas que se usa para golpear suavemente la piel, una práctica que se cree mejora la circulación y abre los poros. Ir a la banya con amigos, colegas o familia —tradicionalmente separados por género— es una costumbre tan arraigada que muchas conversaciones importantes, personales o de negocios, se dan justamente ahí, en un ambiente relajado y sin jerarquías aparentes.",
      },
      en: {
        title: "Sweating Together as a Social Ritual",
        subtitle: "The Banya",
        imageAlt:
          "Cartoon of a wooden sauna with hot stones and a bundle of birch branches hanging nearby.",
        description:
          "The banya, the traditional Russian sauna, is much more than a place to sweat: it's a social ritual that can last hours, alternating between the intense heat of the steam room and cold water from a lake, a pool, or straight into the snow in winter. A central part of the experience is the 'venik,' a bundle of leafy birch or oak branches used to gently swat the skin, a practice believed to improve circulation and open the pores. Going to the banya with friends, colleagues, or family — traditionally separated by gender — is such an ingrained custom that many important conversations, personal or business, happen right there, in a relaxed setting with no apparent hierarchy.",
      },
    },
  },
  {
    id: "metro-moscu",
    order: 7,
    placeholderEmoji: "🚇",
    accentColor: "#8B5E3C",
    imageUrl: null,
    translations: {
      es: {
        title: "Palacios subterráneos para viajar en tren",
        subtitle: "El metro de Moscú",
        imageAlt:
          "Caricatura del andén de una estación de metro decorada con columnas de mármol y candelabros.",
        description:
          "El metro de Moscú, inaugurado en 1935, no fue pensado solo como transporte: fue diseñado como propaganda arquitectónica del régimen soviético, con estaciones decoradas como palacios, llenas de mármol, mosaicos, candelabros de bronce y vitrales que celebran la historia y los logros del país. Estaciones como Komsomólskaya o Mayakóvskaya son visitadas hoy como atracciones turísticas en sí mismas, no solo como paradas de paso. Con más de 250 estaciones y millones de pasajeros por día, sigue siendo uno de los sistemas de transporte más usados del mundo, y probablemente el único donde bajar a esperar un tren se parece a entrar a un museo.",
      },
      en: {
        title: "Underground Palaces You Ride to Work",
        subtitle: "The Moscow Metro",
        imageAlt:
          "Cartoon of a metro station platform decorated with marble columns and chandeliers.",
        description:
          "The Moscow Metro, opened in 1935, wasn't conceived just as transportation: it was designed as architectural propaganda for the Soviet regime, with stations decorated like palaces, full of marble, mosaics, bronze chandeliers, and stained glass celebrating the country's history and achievements. Stations like Komsomolskaya or Mayakovskaya are visited today as tourist attractions in their own right, not just as stops along the way. With more than 250 stations and millions of passengers a day, it remains one of the most heavily used transit systems in the world, and probably the only one where waiting for a train feels like walking into a museum.",
      },
    },
  },
  {
    id: "gran-novela-rusa",
    order: 8,
    placeholderEmoji: "📖",
    accentColor: "#4A4E69",
    imageUrl: null,
    translations: {
      es: {
        title: "Libros tan largos como la vida misma",
        subtitle: "La gran novela rusa",
        imageAlt:
          "Caricatura de una pila alta de libros gruesos y antiguos sobre un escritorio de madera.",
        description:
          "Autores como Liev Tolstói y Fiódor Dostoievski definieron en el siglo XIX un estilo de novela que todavía se estudia como referencia mundial: historias extensas, con decenas de personajes, que combinan drama personal con preguntas filosóficas sobre la culpa, la fe o el sentido de la vida. 'Guerra y paz', de Tolstói, tiene más de 1.200 páginas en muchas ediciones y sigue el destino de varias familias durante las guerras napoleónicas; 'Crimen y castigo', de Dostoievski, narra el colapso psicológico de un asesino desde adentro de su propia cabeza. Leer a los grandes clásicos rusos completos sigue siendo, para muchos lectores en el mundo entero, una especie de rito de paso literario.",
      },
      en: {
        title: "Books as Long as Life Itself",
        subtitle: "The Great Russian Novel",
        imageAlt:
          "Cartoon of a tall stack of thick, old books on a wooden desk.",
        description:
          "Authors like Leo Tolstoy and Fyodor Dostoevsky defined a style of novel in the 19th century that's still studied as a world reference: sprawling stories with dozens of characters, combining personal drama with philosophical questions about guilt, faith, or the meaning of life. Tolstoy's 'War and Peace' runs over 1,200 pages in many editions and follows several families through the Napoleonic wars; Dostoevsky's 'Crime and Punishment' narrates a murderer's psychological collapse from inside his own head. Finishing the great Russian classics remains, for many readers around the world, a kind of literary rite of passage.",
      },
    },
  },
  {
    id: "ded-moroz",
    order: 9,
    placeholderEmoji: "🎄",
    accentColor: "#0B6E4F",
    imageUrl: null,
    translations: {
      es: {
        title: "La fiesta más importante no es Navidad",
        subtitle: "Ded Moroz y Año Nuevo",
        imageAlt:
          "Caricatura de un abuelo de barba blanca con abrigo azul y bastón junto a un árbol decorado.",
        description:
          "En Rusia, el Año Nuevo —no la Navidad— es la fiesta familiar más importante del calendario, una herencia directa de la época soviética, cuando el gobierno oficialmente ateo desalentó las celebraciones religiosas y trasladó buena parte de sus rituales al 31 de diciembre. El árbol decorado, los regalos y hasta la figura equivalente a Papá Noel —'Ded Moroz' (Abuelo Frío), acompañado siempre por su nieta Snegúrochka— se asocian al Año Nuevo, no a la Navidad ortodoxa, que se celebra recién el 7 de enero según el calendario juliano. Las familias suelen quedarse despiertas hasta la medianoche escuchando el discurso presidencial por televisión, brindando con champán apenas suenan las campanadas del Kremlin.",
      },
      en: {
        title: "The Big Holiday Isn't Christmas",
        subtitle: "Ded Moroz and New Year",
        imageAlt:
          "Cartoon of a white-bearded grandfather figure in a blue coat with a staff, next to a decorated tree.",
        description:
          "In Russia, New Year — not Christmas — is the most important family holiday on the calendar, a direct legacy of the Soviet era, when the officially atheist government discouraged religious celebrations and shifted much of their ritual to December 31st. The decorated tree, the gifts, and even the Santa Claus equivalent — 'Ded Moroz' (Grandfather Frost), always accompanied by his granddaughter Snegurochka — are associated with New Year, not Orthodox Christmas, which is celebrated on January 7th under the Julian calendar. Families typically stay up until midnight watching the presidential address on TV, toasting with champagne the moment the Kremlin bells chime.",
      },
    },
  },
  {
    id: "vodka-brindis",
    order: 10,
    placeholderEmoji: "🥂",
    accentColor: "#6C7A89",
    imageUrl: null,
    translations: {
      es: {
        title: "Un trago que se toma con reglas propias",
        subtitle: "Vodka y brindis",
        imageAlt:
          "Caricatura de una botella de vodka helada y pequeños vasos junto a pan negro y pepinillos.",
        description:
          "El vodka es la bebida más asociada a Rusia en todo el mundo, y tomarlo tiene sus propias reglas no escritas: se sirve helado, se toma de un solo trago (nunca a sorbos) y casi siempre va acompañado de algo para comer inmediatamente después, como pan negro, pepinillos encurtidos o arenque. Los brindis, lejos de ser un simple 'salud', suelen ser pequeños discursos improvisados dedicados a la amistad, la familia o los ausentes, y en una cena larga pueden sucederse decenas de ellos a lo largo de la noche. Aunque el consumo bajó fuerte en las últimas dos décadas por campañas de salud pública, el vodka sigue siendo, culturalmente, sinónimo de hospitalidad y celebración compartida.",
      },
      en: {
        title: "A Drink With Its Own Set of Rules",
        subtitle: "Vodka and Toasts",
        imageAlt:
          "Cartoon of a frosty vodka bottle and small glasses next to dark bread and pickles.",
        description:
          "Vodka is the drink most associated with Russia worldwide, and drinking it comes with its own unwritten rules: it's served ice-cold, downed in a single shot (never sipped), and almost always followed immediately by something to eat, like dark bread, pickles, or herring. Toasts, far from a simple 'cheers,' tend to be small improvised speeches dedicated to friendship, family, or absent loved ones, and a long dinner can easily go through dozens of them over the course of the night. Although consumption has dropped sharply over the last two decades due to public health campaigns, vodka remains, culturally, a shorthand for hospitality and shared celebration.",
      },
    },
  },
  {
    id: "tabla-periodica",
    order: 11,
    placeholderEmoji: "🧪",
    accentColor: "#1B4B6B",
    imageUrl: null,
    translations: {
      es: {
        title: "El cuadro que ordenó toda la química",
        subtitle: "La tabla periódica",
        imageAlt: "Caricatura de una grilla de casillas numeradas con símbolos químicos.",
        description:
          "El químico ruso Dmitri Mendeléyev presentó en 1869 la primera versión ampliamente aceptada de la tabla periódica de los elementos, organizando los elementos químicos conocidos hasta entonces según su peso atómico y sus propiedades recurrentes, en un sistema tan preciso que le permitió predecir la existencia de elementos que todavía no se habían descubierto, dejando espacios vacíos en su tabla para ellos. Cuenta la leyenda —probablemente exagerada, pero muy repetida— que la estructura final del ordenamiento se le ocurrió en un sueño, después de días trabajando obsesivamente en el problema. Décadas después, varios de los elementos que Mendeléyev había predicho con huecos vacíos, como el galio y el germanio, fueron efectivamente descubiertos con propiedades casi idénticas a las que él había anticipado, confirmando la validez de su sistema y convirtiéndolo en una de las herramientas más universales y reconocibles de toda la ciencia.",
      },
      en: {
        title: "The Chart That Organized All of Chemistry",
        subtitle: "The Periodic Table",
        imageAlt: "Cartoon of a grid of numbered boxes with chemical symbols.",
        description:
          "Russian chemist Dmitri Mendeleev presented in 1869 the first widely accepted version of the periodic table of elements, organizing the chemical elements known at the time by atomic weight and recurring properties, in a system precise enough to let him predict the existence of elements not yet discovered, leaving empty gaps in his table for them. Legend has it — probably exaggerated, but often repeated — that the final structure came to him in a dream, after days of obsessively working on the problem. Decades later, several of the elements Mendeleev had predicted with empty gaps, like gallium and germanium, were indeed discovered with properties almost identical to what he had anticipated, confirming the validity of his system and making it one of the most universal and recognizable tools in all of science.",
      },
    },
  },
  {
    id: "tetris",
    order: 12,
    placeholderEmoji: "🎮",
    accentColor: "#3D5A80",
    imageUrl: null,
    translations: {
      es: {
        title: "El videojuego creado en una academia de ciencias",
        subtitle: "Tetris",
        imageAlt: "Caricatura de piezas de bloques geométricos cayendo y encastrando entre sí.",
        description:
          "Tetris, uno de los videojuegos más vendidos y reconocibles de la historia, fue creado en 1984 por el programador soviético Alexéi Pázhitnov, que trabajaba en la Academia de Ciencias de la URSS y desarrolló el juego en sus ratos libres usando una computadora Electronika 60, sin ninguna intención comercial inicial. El nombre combina 'tetra' (por las piezas de cuatro bloques) con 'tenis', el deporte favorito de Pázhitnov. Debido a las leyes soviéticas de la época, los derechos del juego pertenecían inicialmente al Estado, y su exportación y licenciamiento a Occidente involucró una negociación tan compleja y opaca que todavía hoy es estudiada como un caso particular de la Guerra Fría aplicada a la industria del entretenimiento. Pázhitnov recién empezó a recibir regalías por su propia creación en 1996, siete años después de que el juego se hubiera vuelto un fenómeno mundial.",
      },
      en: {
        title: "The Video Game Created Inside a Science Academy",
        subtitle: "Tetris",
        imageAlt: "Cartoon of geometric block pieces falling and fitting together.",
        description:
          "Tetris, one of the best-selling and most recognizable video games in history, was created in 1984 by Soviet programmer Alexey Pajitnov, who worked at the USSR Academy of Sciences and developed the game in his spare time using an Electronika 60 computer, with no commercial intention at first. The name combines 'tetra' (for the four-block pieces) with 'tennis,' Pajitnov's favorite sport. Because of Soviet law at the time, the rights to the game initially belonged to the state, and its export and licensing to the West involved a negotiation so complex and opaque that it's still studied today as a particular case of the Cold War applied to the entertainment industry. Pajitnov only started receiving royalties from his own creation in 1996, seven years after the game had already become a worldwide phenomenon.",
      },
    },
  },
  {
    id: "yuri-gagarin",
    order: 13,
    placeholderEmoji: "🚀",
    accentColor: "#0B3D91",
    imageUrl: null,
    translations: {
      es: {
        title: "El primer ser humano en salir de la Tierra",
        subtitle: "Yuri Gagarin",
        imageAlt: "Caricatura de un casco de cosmonauta redondo con una pequeña nave despegando al fondo.",
        description:
          "El 12 de abril de 1961, el cosmonauta soviético Yuri Gagarin se convirtió en el primer ser humano en viajar al espacio, completando una órbita completa alrededor de la Tierra a bordo de la nave Vostok 1 en apenas 108 minutos. Gagarin, hijo de campesinos y ex piloto de caza, fue elegido entre veinte candidatos finalistas en parte por su carisma y su estatura relativamente baja, ideal para la cabina reducida de la nave. Su vuelo se convirtió en un triunfo propagandístico monumental para la Unión Soviética en plena Guerra Fría, y su frase 'Poyejali!' ('¡Vamos!'), pronunciada justo antes del despegue, quedó grabada como una de las citas más recordadas de la historia de la exploración espacial. Gagarin murió en 1968 en un accidente durante un vuelo de entrenamiento rutinario, sin volver a ir al espacio, pero su nombre sigue siendo sinónimo mundial del inicio de la era espacial humana.",
      },
      en: {
        title: "The First Human Being to Leave Earth",
        subtitle: "Yuri Gagarin",
        imageAlt: "Cartoon of a round cosmonaut helmet with a small spacecraft launching in the background.",
        description:
          "On April 12, 1961, Soviet cosmonaut Yuri Gagarin became the first human being to travel into space, completing a full orbit of the Earth aboard the Vostok 1 spacecraft in just 108 minutes. Gagarin, the son of farmers and a former fighter pilot, was chosen among twenty finalist candidates partly for his charisma and his relatively short stature, ideal for the spacecraft's cramped cabin. His flight became a monumental propaganda triumph for the Soviet Union in the middle of the Cold War, and his phrase 'Poyekhali!' ('Let's go!'), said right before liftoff, has gone down as one of the most remembered lines in the history of space exploration. Gagarin died in 1968 in an accident during a routine training flight, never returning to space, but his name remains a worldwide synonym for the dawn of the human space age.",
      },
    },
  },
  {
    id: "tchaikovsky",
    order: 14,
    placeholderEmoji: "🎼",
    accentColor: "#5C4B99",
    imageUrl: null,
    translations: {
      es: {
        title: "El compositor que le puso música a los cuentos de hadas",
        subtitle: "Chaikovski",
        imageAlt: "Caricatura de una batuta de director de orquesta apoyada sobre una partitura abierta.",
        description:
          "Piotr Ilich Chaikovski, el compositor ruso más interpretado en escenarios de todo el mundo, escribió partituras que se volvieron tan universales que muchas personas las reconocen sin saber su origen: 'El lago de los cisnes', 'La bella durmiente' y 'El cascanueces', tres de los ballets más representados de la historia, salen todos de su pluma. La suite de 'El cascanueces' en particular se volvió sinónimo de las fiestas de fin de año en buena parte del mundo occidental, programada casi automáticamente por orquestas y compañías de danza cada diciembre desde hace más de un siglo. Su 'Obertura 1812', compuesta para conmemorar la resistencia rusa contra la invasión napoleónica, incluye disparos de cañón reales en su partitura original y se convirtió en una de las piezas de música clásica más reconocibles fuera del ámbito específicamente clásico, usada hasta en fuegos artificiales y eventos deportivos.",
      },
      en: {
        title: "The Composer Who Put Music to Fairy Tales",
        subtitle: "Tchaikovsky",
        imageAlt: "Cartoon of a conductor's baton resting on an open musical score.",
        description:
          "Pyotr Ilyich Tchaikovsky, the most performed Russian composer on stages around the world, wrote scores that became so universal that many people recognize them without knowing their origin: 'Swan Lake,' 'The Sleeping Beauty,' and 'The Nutcracker,' three of the most performed ballets in history, all come from his pen. The 'Nutcracker' suite in particular became synonymous with year-end holidays across much of the Western world, programmed almost automatically by orchestras and dance companies every December for over a century. His '1812 Overture,' composed to commemorate Russian resistance against Napoleon's invasion, includes actual cannon fire in its original score and became one of the most recognizable pieces of classical music outside strictly classical circles, used everywhere from fireworks displays to sporting events.",
      },
    },
  },
  {
    id: "vanguardia-rusa",
    order: 15,
    placeholderEmoji: "⬛",
    accentColor: "#C1272D",
    imageUrl: null,
    translations: {
      es: {
        title: "El cuadrado negro que quiso terminar con la pintura",
        subtitle: "La vanguardia rusa",
        imageAlt: "Caricatura de formas geométricas simples y planas de colores primarios superpuestas.",
        description:
          "A comienzos del siglo XX, un grupo de artistas rusos —entre ellos Kazimir Malévich, Wassily Kandinsky y Alexander Ródchenko— protagonizó una de las rupturas artísticas más radicales de la historia moderna, dando origen a movimientos como el suprematismo y el constructivismo que influyeron profundamente en el diseño gráfico, la arquitectura y el arte abstracto del siglo XX. La obra más emblemática es probablemente 'Cuadrado negro' (1915), de Malévich: un cuadrado negro pintado sobre fondo blanco, presentado como el punto cero absoluto de la pintura, un rechazo total a representar cualquier objeto reconocible. Aunque el régimen soviético reprimió estas corrientes a favor del realismo socialista desde los años 30, su influencia visual sigue siendo hoy una referencia constante en el diseño gráfico mundial.",
      },
      en: {
        title: "The Black Square That Tried to End All Painting",
        subtitle: "The Russian Avant-Garde",
        imageAlt: "Cartoon of simple, flat geometric shapes in primary colors overlapping each other.",
        description:
          "In the early 20th century, a group of Russian artists — including Kazimir Malevich, Wassily Kandinsky, and Alexander Rodchenko — led one of the most radical artistic breaks in modern history, giving rise to movements like Suprematism and Constructivism that deeply influenced graphic design, architecture, and abstract art throughout the 20th century. The most emblematic work is probably Malevich's 'Black Square' (1915): a black square painted on a white background, presented as the absolute zero point of painting, a total refusal to represent any recognizable object. Although the Soviet regime eventually suppressed these movements in favor of Socialist Realism, their visual influence remains a constant reference in graphic design and advertising worldwide today.",
      },
    },
  },
  {
    id: "eisenstein-montaje",
    order: 16,
    placeholderEmoji: "🎞️",
    accentColor: "#1B1B1B",
    imageUrl: null,
    translations: {
      es: {
        title: "El director que enseñó al cine a cortar",
        subtitle: "Serguéi Eisenstein",
        imageAlt: "Caricatura de tiras de película cortadas y superpuestas sobre una mesa de edición.",
        description:
          "El director soviético Serguéi Eisenstein revolucionó el lenguaje del cine en los años 20 con su teoría del 'montaje', la idea de que el verdadero significado de una película no está en cada toma individual sino en cómo se combinan y chocan entre sí al ser editadas una detrás de otra. Su película 'El acorazado Potemkin' (1925) incluye la secuencia de la escalinata de Odesa, en la que soldados masacran a civiles bajando unas escaleras, considerada una de las secuencias mejor editadas de toda la historia del cine y estudiada en escuelas de cine de todo el mundo. Directores como Alfred Hitchcock, Francis Ford Coppola o Brian De Palma citaron directamente la influencia de Eisenstein en su forma de construir tensión narrativa a través del corte.",
      },
      en: {
        title: "The Director Who Taught Cinema How to Cut",
        subtitle: "Sergei Eisenstein",
        imageAlt: "Cartoon of cut film strips overlapping on an editing table.",
        description:
          "Soviet director Sergei Eisenstein revolutionized cinematic language in the 1920s with his theory of 'montage,' the idea that a film's true meaning doesn't lie in each individual shot but in how shots collide and combine with one another when edited together in sequence. His film 'Battleship Potemkin' (1925) includes the Odessa Steps sequence, in which soldiers massacre civilians on a staircase, still considered one of the best-edited sequences in the history of cinema and required study at film schools worldwide. Directors as different as Alfred Hitchcock, Francis Ford Coppola, and Brian De Palma directly credited Eisenstein's influence on how they build narrative tension through editing.",
      },
    },
  },
  {
    id: "rusia-mas-grande",
    order: 17,
    placeholderEmoji: "🗺️",
    accentColor: "#2E5339",
    imageUrl: null,
    translations: {
      es: {
        title: "Un país tan grande que abarca once husos horarios",
        subtitle: "El país más grande del mundo",
        imageAlt: "Caricatura de un mapa con un territorio enorme cubriendo la mayor parte del globo.",
        description:
          "Rusia es, por lejos, el país más grande del planeta: con más de 17 millones de kilómetros cuadrados, ocupa cerca del 11% de toda la superficie terrestre habitable y es casi el doble de extensa que el segundo país más grande, Canadá. Su territorio abarca once husos horarios distintos, tantos que cuando en Kaliningrado, en el extremo oeste, recién está amaneciendo, en Chukotka, en el extremo este, ya casi está anocheciendo del día siguiente. Esta escala genera situaciones prácticas poco habituales en cualquier otro país: un vuelo interno entre Moscú y Vladivostok puede durar más de ocho horas, y es técnicamente posible celebrar el Año Nuevo once veces distintas dentro del mismo país, a medida que la medianoche va avanzando de este a oeste a lo largo de todo el territorio ruso.",
      },
      en: {
        title: "A Country So Big It Spans Eleven Time Zones",
        subtitle: "The Largest Country in the World",
        imageAlt: "Cartoon of a map with an enormous territory covering most of the globe.",
        description:
          "Russia is, by far, the largest country on the planet: with more than 17 million square kilometers, it covers around 11% of all habitable land on Earth and is nearly twice the size of the second-largest country, Canada. Its territory spans eleven different time zones, so many that when the sun is just rising in Kaliningrad, on the far western edge, it's almost sunset the next day in Chukotka, on the far eastern edge. This scale creates practical situations rare in any other country: a domestic flight between Moscow and Vladivostok can take more than eight hours, and it's technically possible to celebrate New Year eleven separate times within the same country, as midnight sweeps from east to west across the entire Russian territory.",
      },
    },
  },
  {
    id: "caviar",
    order: 18,
    placeholderEmoji: "🥄",
    accentColor: "#1A1A1A",
    imageUrl: null,
    translations: {
      es: {
        title: "Huevas de pescado convertidas en símbolo de lujo",
        subtitle: "El caviar ruso",
        imageAlt: "Caricatura de un pequeño frasco de caviar negro brillante con una cuchara de nácar al lado.",
        description:
          "El caviar, las huevas saladas de esturión, se asocia mundialmente con el lujo y con Rusia en particular gracias a la histórica pesca de esturión en el mar Caspio, compartido con Irán, Kazajistán y otros países vecinos, que durante siglos produjo el caviar más cotizado del planeta, con variedades como el beluga alcanzando precios de miles de dólares por kilo. La sobrepesca y la construcción de represas que interrumpieron las rutas migratorias del esturión llevaron a la especie al borde de la extinción durante el siglo XX, obligando a Rusia y a los países vecinos a imponer estrictas cuotas y, en muchos casos, prohibiciones totales de pesca salvaje. Hoy la mayor parte del caviar 'ruso' que se consume en el mundo proviene en realidad de esturiones criados en granjas de acuicultura, no de pesca silvestre, aunque la asociación cultural entre el caviar, el lujo y Rusia sigue tan firme como siempre en el imaginario popular internacional.",
      },
      en: {
        title: "Fish Eggs Turned Into a Symbol of Luxury",
        subtitle: "Russian Caviar",
        imageAlt: "Cartoon of a small jar of glossy black caviar with a mother-of-pearl spoon beside it.",
        description:
          "Caviar, the salted eggs of sturgeon, is associated worldwide with luxury and with Russia in particular thanks to the historic sturgeon fishery of the Caspian Sea, shared with Iran, Kazakhstan, and other neighboring countries, which for centuries produced the most prized caviar on the planet, with varieties like beluga reaching prices of thousands of dollars per kilo. Overfishing and dam construction that disrupted sturgeon migration routes drove the species to the brink of extinction during the 20th century, forcing Russia and neighboring countries to impose strict quotas and, in many cases, total bans on wild fishing. Today most of the 'Russian' caviar consumed around the world actually comes from sturgeon raised on aquaculture farms rather than wild fishing, though the cultural association between caviar, luxury, and Russia remains as strong as ever in the international popular imagination.",
      },
    },
  },
  {
    id: "lago-baikal",
    order: 19,
    placeholderEmoji: "🌊",
    accentColor: "#1D6FA5",
    imageUrl: null,
    translations: {
      es: {
        title: "El lago con una quinta parte del agua dulce del planeta",
        subtitle: "El lago Baikal",
        imageAlt: "Caricatura de un lago profundo y azul rodeado de montañas nevadas.",
        description:
          "El lago Baikal, en el sur de Siberia, es el lago de agua dulce más profundo y más antiguo del mundo, con más de 1.600 metros de profundidad máxima y una edad estimada de al menos 25 millones de años, muchísimo más viejo que la inmensa mayoría de los lagos del planeta, que suelen desaparecer geológicamente en apenas algunos miles de años. Contiene aproximadamente el 20% de toda el agua dulce no congelada disponible en la superficie terrestre, un volumen tan grande que, si de repente desaparecieran todos los demás lagos del mundo, el agua del Baikal sola tardaría casi un año en llenar el hueco. Su aislamiento geológico durante millones de años permitió la evolución de miles de especies endémicas que no existen en ningún otro lugar, entre ellas la nerpa o foca del Baikal, la única especie de foca del mundo que vive exclusivamente en agua dulce, a miles de kilómetros del océano más cercano.",
      },
      en: {
        title: "The Lake That Holds a Fifth of the World's Fresh Water",
        subtitle: "Lake Baikal",
        imageAlt: "Cartoon of a deep blue lake surrounded by snowy mountains.",
        description:
          "Lake Baikal, in southern Siberia, is the world's deepest and oldest freshwater lake, with a maximum depth of more than 1,600 meters and an estimated age of at least 25 million years, far older than the vast majority of lakes on the planet, which typically vanish geologically within just a few thousand years. It holds roughly 20% of all the unfrozen fresh surface water on Earth, a volume so large that if every other lake in the world suddenly vanished, Baikal's water alone would take nearly a year to fill the gap. Its geological isolation over millions of years allowed thousands of endemic species to evolve that exist nowhere else, including the Baikal seal, or nerpa, the world's only seal species that lives exclusively in fresh water, thousands of kilometers from the nearest ocean.",
      },
    },
  },
  {
    id: "kizhi-pogost",
    order: 20,
    placeholderEmoji: "⛪",
    accentColor: "#8B7355",
    imageUrl: null,
    translations: {
      es: {
        title: "Una iglesia de 22 cúpulas construida sin un solo clavo",
        subtitle: "Kizhi Pogost",
        imageAlt: "Caricatura de una iglesia de madera con múltiples cúpulas escalonadas y bulbosas.",
        description:
          "En la isla de Kizhi, en el lago Onega, al norte de Rusia, se levanta la Iglesia de la Transfiguración, una estructura de madera de casi 37 metros de altura coronada por 22 cúpulas bulbosas, construida en 1714 según la tradición local sin usar un solo clavo de metal, ensamblando todas sus piezas mediante técnicas de carpintería tradicional rusa de encastre y talla. La leyenda popular, imposible de confirmar históricamente, cuenta que el carpintero principal tiró su hacha al lago al terminar la obra, declarando que nunca más se construiría algo igual. Declarado Patrimonio de la Humanidad por la UNESCO en 1990, el conjunto arquitectónico de Kizhi requirió, décadas después, una restauración estructural interna con soportes de acero ocultos para garantizar su estabilidad a largo plazo, aunque manteniendo intacta su apariencia exterior original de madera tallada sin un solo clavo visible.",
      },
      en: {
        title: "A 22-Domed Church Built Without a Single Nail",
        subtitle: "Kizhi Pogost",
        imageAlt: "Cartoon of a wooden church with multiple tiered, bulbous domes.",
        description:
          "On the island of Kizhi, on Lake Onega in northern Russia, stands the Church of the Transfiguration, a wooden structure nearly 37 meters tall crowned by 22 bulbous domes, built in 1714 according to local tradition without using a single metal nail, assembling every piece using traditional Russian joinery and carving techniques. A popular legend, impossible to confirm historically, claims the master carpenter threw his axe into the lake upon finishing the work, declaring that nothing like it would ever be built again. Declared a UNESCO World Heritage Site in 1990, the Kizhi architectural ensemble required, decades later, an internal structural restoration with hidden steel supports to guarantee its long-term stability, while keeping its original exterior appearance of carved wood with not a single visible nail intact.",
      },
    },
  },
];
